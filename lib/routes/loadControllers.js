...
function parseRoute(route, routePath, isDefault) {
	...
	if (isDefault) {
		if (route.type !== undefined) throw new OverlookError("Default routes should not have type defined (in '" + routePath.join('.') + "')");
	} else {
		if (route.type === undefined) {
			// undefined route type
			throw new OverlookError("Route '" + routePath.join('.') + "' has no type defined");
		} else if (routeTypes.indexOf(route.type) == -1) {
			// invalid route type
			throw new OverlookError("Route '" + routePath.join('.') + "' has invalid type");
		}
	}

	// promisify any route methods which are generators
	promisify.generators(route);
	...
	_.forIn(route.actions, function(action, actionName) {
		...
		if (action !== false && !_.isPlainObject(action)) throw new OverlookError("Route action '" + routePath.join('.') + '.' + actionName + "' is not valid");
		...
		// promisify any action methods which are generators
		promisify.generators(action);
	});
	...
	if (route.routes && route.routes._default) {
		...
		if (!_.isEmpty(route.routes._default.actions)) {
			...
			_.forIn(route.defaultActions, function(action, actionName) {
				if (action.actionTypes) throw new OverlookError("Action types should not be defined in default actions in '" + routePath.join('.') + '.' + actionName + "'");
			});
		}
