...
route = {
	...
	nameSingular: <routeName singular> (default: name of folder singularized)
	...
	title: <human-readable name of resource/namespace> (default: name humanized)
	titleItem: <human-readable name of item> (default: routeNameSingular humanized)

	view: <base dir of view> (default: <corresponding to directory path of route>)

	modelName: name of model to load by default (default: nameSingular capitalized)
...
action = {
	...
	title: <human-readable name of action> (default: name capitalized)
...
module.exports = function(models, overlook, controllersPath, viewsPath) {
	...
	overlook.log('Routes initializing');
	var routes = loadControllers(controllersPath);

	overlook.log('Routes loaded from files');

	// apply defaults
	applyDefaults(routes);

	overlook.log('Route defaults applied');
