...
function initRoute(route, parent) {
	...
	if (route.type == 'resource' || route.type == 'resourceJoin') {
		...
		if (!route.modelName) {
			if (route.parentType != 'resource') {
				...
				_.forIn(parent.model.associations, function(thisAssociation) {
					if (['HasMany', 'BelongsToMany'].indexOf(thisAssociation.associationType) != -1 && thisAssociation.as == route.as) {
		...
		if (!route.model) throw new OverlookError("Model '" + route.modelName + "' is not defined in " + route.sourcePath);

		// record modelPath
		// uses modelPathSolos to determine which is best route (routes with namespace parent take precedence)
		//xxx Using '' as path is a hack to make links in nested resource and resourceJoin index tables work. Find a better way.
		if (!modelPathSolos[route.modelName]) {
			modelPaths[route.modelName] = ((route.parentType == 'resource') ? '' : route.path);
			if (route.parentType != 'resource') modelPathSolos[route.modelName] = true;
		}
	} else {
		...
		_.forEach(['param', 'paramField', 'modelName'], function(illegal) {
			if (route[illegal] !== undefined) throw new OverlookError('Routes which are not resource/resourceJoin should not have ' + illegal + ' defined in ' + route.sourcePath);
		});
	}
...
function createRouteActions(route, isItem) {
	...
	_.forIn(route.actions, function(action) {
		...
		overlook.log('Route initialized', {path: action.path});
	});
}
