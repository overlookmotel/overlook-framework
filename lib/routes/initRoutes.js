...
function initRouteActions(route) {
	...
	return Promise.forIn(route.actions, function(action, actionName) {
		...
		// sort actions with item actions after non-item actions
		//xxx is this still neccesary as item routes are run after now anyway?
		_.sort(route.actions, function(a, b) {
			if (a.value.actionTypes.item && !b.value.actionTypes.item) return true;
			if (!a.value.actionTypes.item && b.value.actionTypes.item) return false;
