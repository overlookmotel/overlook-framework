...
module.exports = {
    ...
    startup: function(overlook) {
        return Promise.in(overlook.workers, function(worker) {
            var currentJob = new worker.Job(worker);

            return currentJob.apiHit('current', {workerId: currentJob.Worker.workerId}).bind(this)
            .tap(function(results) {
                if (!results) throw new OverlookError('Cannot connect to API server');
            })
            .eachParallel(function(jobParams) {
                // call failed method of job
                var job = new worker.Job(worker);
                job.jobId = jobParams.id;
...
_.extend(Job.prototype, {
    ...
    success: function(result) {
        ...
        .then(function() {
            ...
            return this.apiHit('success', result);
        });
    },
