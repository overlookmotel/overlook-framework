...
exports = module.exports = {
	...
	stock: function(sequelize) {
		...
		return Promise.try(function() {
			// create system user
			return models.user.findOrCreate({where: {isSystem: true}, defaults: {name: 'System', email: 'system@system.com', isActive: false}})
			...
			.spread(function(hash, key) {
				return models.user.findOrCreate({where: {isRoot: true}, defaults: {name: 'Root', email: 'root@root.com', passwordHash: hash, cookieKey: key, createdById: systemUser.id, updatedById: systemUser.id}});
			}).spread(function(user) {
				rootUser = user;
			});
		}).then(function() {
			// create public user
			return models.user.findOrCreate({where: {isPublic: true}, defaults: {name: 'Public', email: 'public@public.com', isActive: false, createdById: systemUser.id, updatedById: systemUser.id}})
			.spread(function(user) {
				publicUser = user;
			});
		}).then(function() {
