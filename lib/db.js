...
exports = module.exports = {
	...
	stock: function(sequelize) {
		...
		return Promise.forIn(users, function(userParams, userName) {
			...
			// create permissions
			return Promise.forIn(permissions, function(permissionParams, permissionName) {
				permissionParams.createdById = permissionParams.updatedById = users.system.id;
			...
			return Promise.all([
				...
				roles.root.addPermissions([permissions.root, permissions.admin, permissions.user], {createdById: users.system.id, updatedById: users.system.id}),
				roles.admin.addPermissions([permissions.admin], {createdById: users.system.id, updatedById: users.system.id}),
				roles.user.addPermissions([permissions.user], {createdById: users.system.id, updatedById: users.system.id})
			]);
		}).then(function() {
