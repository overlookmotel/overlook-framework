...
module.exports = {
	...
	init: function(options) {
		...
		_.defaults(paths, {
			...
			workers: pathModule.join(paths.root, './workers')
		});
		...
		_.defaultValue(options, 'workers', {});
		workers.init(this);
	...
	start: function() {
		...
		return Promise.bind(this).then(function() {
			...
			// clean-up workers after crash/shutdown
			console.log('Cleaning up after workers');

			return workers.startup(this);
		}).then(function() {
			// start server
			...
			//xxx not starting up automatically for now
			_.forIn(this.workers, function(worker) {
				worker.start();
			});
