...
module.exports = {
	...
	init: function(options) {
		...
		// set up logging
		this.log = logger(options.name, paths.log);

		// log application starting event
		this.log('Application starting', {name: options.name, user: whoami(), userId: process.getuid()});
	...
	startDB: function() {
		...
		return this.sequelize.sync().bind(this)
		.then(function() {
			this.log('Database synced');

			// initialise public and root users
			this.log('Initialising system, root and public users');
			...
			.spread(function(users, roles) {
				...
				this.log('Initialised system, root and public users');
			});
		}).then(function() {
			this.dbSynced = true;
	...
	start: function() {
		...
		.catch(function(err) {
			this.log.fatal('Error during application start up', {err: err});
