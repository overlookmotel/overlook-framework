...
var forms = module.exports = {
	...
	populateFromModelInstance: function(form, item) {
		...
		_.forIn(form.fields, function(field, fieldName) {
			...
			if (field.additionalFields) {
				...
				if (!item.hasOwnProperty(dataName)) throw new errors.OverlookError('Form requires ' + dataName + ' to be loaded'); //xxx check this correctly identifies unloaded items
				var addItem = item[dataName];
				
				_.forEach(field.additionalFields, function(addFieldName) {
					if (addItem !== undefined && addItem !== null && addItem[addFieldName] === undefined) throw new errors.OverlookError('Form requires ' + dataName + '.' + addFieldName + ' to be loaded');
	...
	populateField: function(field, value) {
		...
		_.forIn(field.populate, function(options, populatorName) {
			...
			if (!populator) throw new errors.OverlookError('Populator ' + populatorName + ' does not exist');
	...
	sanitise: function(form, formData) {
		_.forIn(form.fields, function(field, fieldName) {
			_.forIn(field.sanitise, function(options, sanitiserName) {
				...
				if (!sanitiser) throw new errors.OverlookError('Sanitiser ' + sanitiserName + ' does not exist');
	...
	validateField: function(value, fieldName, field, formData) {
		...
		_.forIn(field.validate, function(options, validatorName) {
			...
			if (!validator) throw new errors.OverlookError('Validator ' + validatorName + ' does not exist');
	...
	conformField: function(field, value) {
		...
		_.forIn(field.conform, function(options, conformerName) {
			...
			if (!conformer) throw new errors.OverlookError('Conformer ' + conformerName + ' does not exist');
	...
	formatInherit: function(field) {
		...
		if (!fieldFormat) throw new errors.OverlookError('Field format ' + format + ' is not defined');
