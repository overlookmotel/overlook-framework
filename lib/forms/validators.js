...
var validators = module.exports = {
	...
	url: function(val, options, fieldName, field, formData) { // jshint ignore:line
		...
		if (match[1] && ['http', 'https', 'ftp'].indexOf(match[1]) == -1) return '%label must be an http://, https:// or ftp:// URL';
	},
	twitter: function(val, options, fieldName, field, formData) { // jshint ignore:line
		if (!val.match(/^@?[\w\-]+$/)) return '%label must be a valid Twitter handle';
	},
	...
	json: function(val, options, fieldName, field, formData) { // jshint ignore:line
		try {
			var obj = JSON.parse(val); // jshint ignore:line
		} catch (err) {
			return '%label must be valid JSON';
		}
	},

	intCollection: function(val, options, fieldName, field, formData) { // jshint ignore:line
		if (!val.match(/^\d+(\s*,\s*\d+)*$/)) return '%label must be a comma-separated string of numbers';
	}
};
