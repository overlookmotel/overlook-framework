...
exports = module.exports = {
	...
	processHttpAuth: function(req, overlook) {
		...
		return overlook.models.server.find({attributes:['passwordHash'], where: {id: serverId, isActive: true}}).bind(this)
	...
	//xxx only used in `runRouteRemote()` which is currently commented out - can be deleted
	getUser: function(userId, overlook) {
		return overlook.models.user.find({where: {id: userId, isActive: true}, attributes: ['name', 'isInitialized']})
		.then(function(user) {
			if (!user) return;
